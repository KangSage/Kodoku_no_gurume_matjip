...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(HttpSecurity http) throws Exception {
    ...
    http.authorizeRequests()
        .antMatchers("/", "/j_spring_security_check", "/user/register", "/html/*", "/assets/**",
            "/actuator/**")
        .permitAll().antMatchers("/admin/**", "/html/admin/**").hasAnyRole("ADMIN")
        .antMatchers("/user/**", "/html/user/**").hasAnyRole("USER").anyRequest().authenticated()
        .and().headers().frameOptions().disable().and().exceptionHandling()
        .authenticationEntryPoint(new LoginUrlAuthenticationEntryPoint("/html/login.html")).and()
        .formLogin().loginPage("/html/login.html").loginPage("/html/admin-login.html")
        .loginProcessingUrl("/j_spring_security_check").usernameParameter("email")
        .passwordParameter("password").successHandler(this.getLoginSuccessHandler())
        .failureHandler(this.getLoginFailureHandler()).and().logout()
        .logoutUrl("/j_spring_security_logout").permitAll()
        .logoutSuccessHandler(this.getLogoutSuccessHandler())
        .deleteCookies("JSESSIONID", REMEMBER_ME_COOKIE_KEY).invalidateHttpSession(true).and()
        .rememberMe().key(REMEMBER_ME_ENC_KEY) // default: uuid random -> some times provider can't
                                               // read these
        // value, please set constant value
        .rememberMeServices(this.getTokenBasedRememberMeServices())
        .tokenValiditySeconds(60 * 60 * 24 * 14).and()
